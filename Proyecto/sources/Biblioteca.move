...
module account::biblioteca {
	...
	public entry fun agregar_libro(    // funcion para agregar libros 
		account: &signer,
		titulo: String,
		autor: String,
		editorial: String,
		...
		assert!(exists<Biblioteca>(address_of(account)),NO_INICIALIZADO);  // Validando que este inicializado
        
        let libros = borrow_global_mut<Biblioteca>(address_of(account));
        assert!(!table::contains(&libros.libros, Titulo { titulo }), REGISTRO_YA_EXISTE); // validando si ya existe el titulo

        table::add(&mut libros.libros, Titulo {   //luego de validar pocedemos a agregar el libro
            titulo,
        }, Libro {
            autor,
            editorial,
...
public fun obtener_libro(account: address, titulo: String): Libro acquires Biblioteca {
	...
	let resultado = table::borrow(&libros.libros, Titulo {titulo}); // mostramos los libros creados
	*resultado

}
// procedemos a crear diversas funciones para modificar ciertas variables del libro
